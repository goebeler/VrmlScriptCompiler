...
void ArgumentList::add_argument(Expression *e) { m_arguments.push_back(e); }
...
std::string VariableExpression::to_string() const { return m_name; }

std::string VariableDeclarationExpression::to_string() const {
  if (m_initializer)
    return m_name + " = " + m_initializer->to_string();
  return m_name;
}

void VariableExpression::accept(ASTVisitor &visitor) { visitor.visit(this); }
...
void IntConstantExpression::accept(ASTVisitor &visitor) { visitor.visit(this); }

std::string FunctionCallExpression::to_string() const {
  if (m_this && !m_function_name.empty()) {
    return fmt::format("{}.{}({})", m_this->to_string(), m_function_name,
                       m_argument_list ? m_argument_list->to_string() : "");
  }
  // free call: name(args)
  if (m_callee == nullptr && !m_function_name.empty()) {
    return fmt::format("{}({})", m_function_name,
                       m_argument_list ? m_argument_list->to_string() : "");
  }
  // callee expression call: calleeExpr(args)
  if (m_callee) {
    return fmt::format("{}({})", m_callee->to_string(),
                       m_argument_list ? m_argument_list->to_string() : "");
  }
  return "()<unknown>";
}
...
std::string Script::to_string() const { return std::string(); }

void Script::accept(ASTVisitor &visitor) { visitor.visit(this); }
...
std::string BinaryArithmeticExpression::to_string() const {
  ...
  return fmt::format("{0} {1} {2}", m_lhs->to_string(), operator_sign,
                     m_rhs->to_string());
}
...
struct add_visitor {
  vrmlscript::VrmlVariant operator()(vrmlscript::SFInt32 int_in,
                                     vrmlscript::SFString string_in) const {
    return vrmlscript::VrmlVariant{
        fmt::format("{0}{1}", int_in, string_in.to_string())};
  }

  vrmlscript::VrmlVariant operator()(vrmlscript::SFInt32 int_in,
                                     vrmlscript::SFInt32 int2_in) const {
...
std::string IndexExpression::to_string() const {
  const auto base =
      m_collection_expression ? m_collection_expression->to_string() : "<null>";
  const auto idx =
      m_index_expression ? m_index_expression->to_string() : "<null>";
...
std::string MemberAccessExpression::to_string() const {
  const auto base = m_object ? m_object->to_string() : "<null>";
  return fmt::format("{}.{}", base, m_member_name);
}
